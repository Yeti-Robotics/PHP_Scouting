...
<?php
include ("connect.php");
$query = "SELECT t1.team AS Team, ROUND(t1.avg_height,2) AS 'Highest Stack Made', ROUND(t2.avg_stacks,2) AS 'Avg. Stacks per Match', ROUND(t1.avg_height * t2.avg_stacks,2) AS 'Avg. Totes per Match', ROUND(rating,2) AS 'Rating'
FROM (SELECT team, MAX(totes) AS avg_height
FROM stacks
